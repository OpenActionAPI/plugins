...
def process_images(input_dir, preferred_format, force_reformat, 
                  png_quality=99, jpeg_quality=95, webp_quality=90, 
                  use_palette=True, palette_colors=256):
    total_files = 0
    modified_files = 0
    original_size = 0
    new_size = 0
    ...
    if preferred_format.lower() not in format_extensions:
        logging.error(f"Unsupported format: {preferred_format}. Supported formats are: {', '.join(format_extensions.keys())}")
        return
        ...
        try:
            with Image.open(filepath) as img:
                ...
                needs_processing = (
                    force_reformat or 
                    needs_resize or 
                    Path(filename).suffix.lower() != target_ext or
                    original_format != preferred_format.lower()
                )
...
def main():
    parser = argparse.ArgumentParser(description='Process images in ./icons directory to max 144x144 resolution.')
    ...
    # Quality settings
    parser.add_argument('--png-quality', type=int, default=99,
    ...
    # Palette settings
    parser.add_argument('--no-palette', action='store_false', dest='use_palette',
